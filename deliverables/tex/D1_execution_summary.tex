...
\vspace{0.5em}
\noindent{\small The harness repository contains every run log and manifest cited
below, the patched LoMix source alongside its unmodified \code{.orig} backups,
the environment setup, and the analysis tools. Datasets, pretrained weights and
checkpoints are excluded by design; \code{setup/02\_data\_and\_weights.md}
...
Epochs & 300$^\ast$ & 20 & \textbf{Under-trained --- the main caveat on every Dice below} \\
...
$^\ast$The paper's main text does not state an epoch budget; 300 is the default
in \code{train\_synapse\_lomix.py}. It is cited here as the repository's
schedule, not as a published figure.

Everything here is a \textbf{reduced-budget reproduction at 20 of the
repository's default 300 epochs}. No Dice below should be compared with a
published number without repeating that sentence. The paper's own results are
averaged over at least three runs; mine are a single seed.
...
The paper states this qualitatively rather than numerically --- ``the added
parameters are negligible and used only during training, leaving the FLOPs,
latency, and memory footprint at test-time unchanged'' (\S1) --- and gives no
inference-cost table in the main text, so there is no published figure to
compare against. The measurement above is therefore a direct confirmation of the
claim rather than a reproduction of a reported number: parameter count, GMACs
and peak inference VRAM are identical across all four supervision settings.
Absolute latency is not comparable in any case, since the paper's experiments
use an RTX A6000 (48\,GB) and these use a GTX 960.
...
full-schedule run, which this hardware cannot deliver in the available time.
...
\subsection{Side-by-side with the published numbers}

The paper's Table~1 reports exactly these four arms on exactly this backbone
(PVT-EMCAD-B2, Synapse 8-organ), which makes the comparison direct. The left
block is \textbf{the paper's}, averaged over at least three runs on an RTX
A6000; the right block is \textbf{mine}, one seed at 20 epochs on a GTX 960.
These are not like-for-like and the gap should not be read as a contradiction.
...
\textbf{Arm} & \textbf{DICE} & \textbf{HD95} & \textbf{mIoU} & &
\textbf{DICE} \\
\midrule
\code{last\_layer} (LL) & 80.9 & 22.9 & 71.2 & & 67.6 \\
\code{deep\_supervision} (DS) & 82.9 & 19.7 & 73.8 & & \textbf{79.8} \\
\code{mutation} & 83.6 & 15.7 & 74.7 & & 77.9 \\
\code{lomix} & \textbf{85.1} & \textbf{14.9} & \textbf{76.4} & & 72.3$^\dagger$ \\
\bottomrule
\end{tabular}
\end{center}
{\small All values are percentages. $^\dagger$best before divergence at epoch
13, not a completed run.}

Three things follow.

\begin{enumerate}
\item \textbf{The published ordering is monotone
(LL $<$ DS $<$ mutation $<$ LoMix); mine is not.} The paper reports $+4.2$ DICE
for LoMix over LL, $+2.2$ over DS and $+1.5$ over unweighted mutation, with
two-sided Wilcoxon signed-rank tests confirming significance over LL and DS.
My runs reproduce the sign of the LL $\rightarrow$ DS step ($+12.2$, larger than
the paper's $+2.0$) but not the DS $\rightarrow$ mutation $\rightarrow$ LoMix
steps.
\item \textbf{Absolute values are uniformly lower}, by 3--13 DICE. This is what
20 epochs against a converged schedule looks like, and my best arm (79.8) sits
just below the paper's \emph{weakest} arm (80.9) --- consistent with
under-training rather than with a broken setup.
\item \textbf{The margins I would need to resolve are small.} The paper's
DS $\rightarrow$ LoMix gap is 2.2 DICE, established over at least three runs
with a signed-rank test. A single seed at 20 epochs cannot separate effects of
that size, which is why \S8.1 treats my ordering as unresolved rather than
contradictory.
\end{enumerate}
...
All of it is in the harness repository: raw run logs and per-sweep manifests in
\code{logs/}, the patch script and the untouched \code{trainer.py.orig} for a
line-level diff in \code{repos/LoMix/}, and the analysis tools
(\code{collect\_results.py}, \code{diagnose\_loss\_ops.py}, \code{memfit.py},
\code{bench\_efficiency.py}, \code{check\_data.py}, \code{smoke\_test.py}) in
\code{tools/}. Every table in this document can be regenerated from those logs
with \code{collect\_results.py}.
